...
export default function ModeCard({ mode, onClick }: Props) {
  ...
  return (
    ...
    >
      <div className="text-xs text-white/40 font-mono uppercase tracking-widest mb-2">
        {label}
      </div>
      <div className="text-2xl font-mono text-white group-hover:text-white transition-colors">
        {description}
      </div>
      <div className="mt-4 text-xs text-white/30 group-hover:text-white/50 transition-colors font-mono">
        Play →
      </div>
    </button>
  );
